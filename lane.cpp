...
void Lane::setBlockage(double percent) {
    blocked = true;
    blockagePercent = percent;
    cout << "🚧 " << laneName << " lane blocked at " 
         << (int)(percent * 100) << "%" << endl;
}
...
int Lane::getEffectiveCapacity() const {
    if (blocked) {
        return (int)(capacity * (1.0 - blockagePercent));
    }
    return capacity;
}
...
void Lane::displayLaneStatus() const {
    cout << left << setw(10) << laneName 
         << " | Vehicles: " << setw(3) << getVehicleCount()
         << " | Avg Wait: " << setw(4) << getAverageWaitingTime() << "s"
         << " | Priority: " << fixed << setprecision(2) 
         << calculatePriority();
    
    if (blocked) {
        cout << " |  BLOCKED (" << (int)(blockagePercent * 100) << "%)";
    }
    if (hasEmergencyVehicle()) {
        cout << " |  EMERGENCY";
    }
